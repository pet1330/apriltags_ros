...
void AprilTagDetector::imagePicture(const sensor_msgs::ImageConstPtr& msg){
  ...
  BOOST_FOREACH(AprilTags::TagDetection detection, detections){
    ...
    Eigen::Matrix4d transform = detection.getRelativeTransform(tag_size, fx, fy, px, py);
        ...
        // subject 0.025m from the tags Z orientation centre of the block being marked)
        Eigen::Affine3d minus_z;
        minus_z.setIdentity();
        // minus_z.translation()<<0,0,-0.01625;
        minus_z.translation()<<0,0,-0.025;
        transform=transform*minus_z.matrix();
